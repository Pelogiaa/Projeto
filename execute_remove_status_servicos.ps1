# Script PowerShell para executar a remocao da coluna status da tabela tab_servicos
# Executa o script SQL remove_status_servicos.sql

Write-Host "=== REMOCAO DA COLUNA STATUS DA TABELA TAB_SERVICOS ===" -ForegroundColor Yellow
Write-Host ""
...
Write-Host "Configuracoes do banco:" -ForegroundColor Cyan
...
$SQL_FILE = "remove_status_servicos.sql"
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "SUCESSO: Coluna status removida da tabela tab_servicos!" -ForegroundColor Green
        Write-Host ""
        ...
        Write-Host "4. Atualizar o ServicoController.java no backend" -ForegroundColor White
        Write-Host "5. Atualizar o ServicoRepository.java no backend" -ForegroundColor White
        Write-Host "6. Recompilar e reiniciar o backend" -ForegroundColor White
    } else {
...
Write-Host ""
Write-Host "=== SCRIPT CONCLUIDO ===" -ForegroundColor Yellow